...
export default function CuttedStockTable() {
    ...
    async function handleSell(e) {
        e.preventDefault()
        setItemSelected({
            soldCompany: sellingCompany.current.value
        })
        console.log('Reached here and item selected is '+itemSelected)
        const CuttingRef = firestore.collection('/Item Cut')
        const docRef = CuttingRef.doc(itemSelected['id'])
        await docRef.update({
            Sold: true,
            Sell_Company: itemSelected.soldCompany
        })
    }
            ...
            <div id="modal1" className="modal">
                <form>
                    ...
                    <div className="modal-footer">
                        ...
                        <button type='submit' onClick={() => handleSell(e)} className="modal-close waves-effect waves-green btn-flat">Sell</button>
                    </div>
                </form>
            </div>
            <div className="App">
